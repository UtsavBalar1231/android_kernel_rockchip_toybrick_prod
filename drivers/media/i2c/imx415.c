...
#define IMX415_XVCLK_FREQ_37M	37500000//	37125000 //csqerr
...
{
	if (imx415->is_thunderboot ) { //&& rkisp_tb_get_state() == RKISP_TB_NG) {//jost for thunderboot
...
static int imx415_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		if (imx415->is_thunderboot) {// && rkisp_tb_get_state() == RKISP_TB_NG) { //just for thunderboot
			imx415->is_thunderboot = false;
			__imx415_power_on(imx415);
		}
...
int __imx415_power_on(struct imx415 *imx415)
{
	...
	if (imx415->is_thunderboot)//csqerr
		return 0;
			...
			const struct i2c_device_id *id)
{
	...
	ret = media_entity_init(&sd->entity, 1, &imx415->pad, 0);
	if (ret < 0)
		goto err_power_off;
#endif
