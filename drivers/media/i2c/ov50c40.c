...
#define MEDIA_BUS_FMT_SPD_2X8           0x5003//csqerr

static const struct other_data ov50c40_spd = {
	.width = 1016,
	.height = 760,
	.bus_fmt = MEDIA_BUS_FMT_SPD_2X8,//csqerr
	.data_type = 0x19,
	.data_bit = 10,
};
...
static int ov50c40_g_mbus_config(struct v4l2_subdev *sd,
				  ...
				  struct rkmodule_inf *inf)
{
	...
	//if (otp)
		//ov50c40_get_otp(otp, inf);//csqerr
}
...
static int __ov50c40_start_stream(struct ov50c40 *ov50c40)
{
	...
	mutex_unlock(&ov50c40->mutex);
	ret = v4l2_ctrl_handler_setup(&ov50c40->ctrl_handler);
	mutex_lock(&ov50c40->mutex);
...
static int ov50c40_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		if (ov50c40->is_thunderboot) {// && rkisp_tb_get_state() == RKISP_TB_NG) {//just for thunderboot
...
static const struct v4l2_subdev_video_ops ov50c40_video_ops = {
	.g_mbus_config = ov50c40_g_mbus_config,
			...
			const struct i2c_device_id *id)
{
	...
	ret = v4l2_fwnode_endpoint_parse(of_fwnode_handle(endpoint),
					 &ov50c40->bus_cfg);
	if (ov50c40->bus_cfg.bus_type == V4L2_MBUS_CSI2) {
...
continue_probe:
	...
	ret = media_entity_init(&sd->entity, 1, &ov50c40->pad, 0);
	if (ret < 0)
		goto err_power_off;
#endif
