			...
			const struct i2c_device_id *id)
{
	...
	unsigned int max_ma = 0, start_ma, rated_ma, step_mode;
	...
	ret = media_entity_init(&aw8601_dev->sd.entity, 0, NULL, 0);
	if (ret < 0)
		goto err_cleanup;

	sd = &aw8601_dev->sd;
	sd->entity.type = MEDIA_ENT_T_V4L2_SUBDEV_LENS;//csqerr
	...
	aw8601_dev->start_move_tv = ns_to_timeval(ktime_get_ns());
	aw8601_dev->end_move_tv = ns_to_timeval(ktime_get_ns());
